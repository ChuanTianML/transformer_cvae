...
_TARGET_THRESHOLD = 300  # Accept vocabulary if size is within this threshold
VOCAB_FILE = "vocab.30k"
...
def main(unused_argv):
  ...
  vocab_file = os.path.join(FLAGS.raw_dir, VOCAB_FILE)
  train_files = {}
  for k in _TRAIN_DATA_SOURCES:
    train_files[k] = os.path.join(FLAGS.raw_dir, _TRAIN_DATA_SOURCES[k])
  eval_files = {}
  for k in _EVAL_DATA_SOURCES:
    eval_files[k] = os.path.join(FLAGS.raw_dir, _EVAL_DATA_SOURCES[k])

  # Create subtokenizer based on the given vocabulary.
  tf.logging.info("Step 1/2: Creating subtokenizer and building vocabulary")
  #subtokenizer = tokenizer.Subtokenizer(vocab_file)
  #train_files_flat = (train_files["input"], train_files["target"])
  train_files_flat = [os.path.join(FLAGS.raw_dir, CORPUS_TRAIN)]
  ...
  # DEBUG: check vocab
  if False:
    tmp_vocab_file = open('tmp_vocab.txt', 'w')
    for t in subtokenizer.subtoken_list:
    #for t in subtokenizer.alphabet:
        tmp_vocab_file.write(t.encode('utf-8')+'\n')
    tmp_vocab_file.close()

  # Tokenize and save data as Examples in the TFRecord format.
  tf.logging.info("Step 2/2: Preprocessing and saving data")
  compiled_train_files = (train_files["input"], train_files["target"])
  compiled_eval_files = (eval_files["input"], eval_files["target"])
  train_tfrecord_files = encode_and_save_files(
      subtokenizer, FLAGS.data_dir, compiled_train_files, _TRAIN_TAG,_TRAIN_SHARDS)
...
def define_data_download_flags():
  ...
  flags.DEFINE_string(
      name="data_dir", short_name="dd", default="/tmp/transformer_data",
      help=flags_core.help_wrap(
          "Directory for where the translate_ende_wmt32k dataset is saved."))
  flags.DEFINE_string(
      name="raw_dir", short_name="rd", default="/tmp/transformer_raw",
      help=flags_core.help_wrap(
