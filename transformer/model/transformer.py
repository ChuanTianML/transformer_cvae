...
class Transformer(object):
  ...
  def __call__(self, inputs, targets=None):
    ...
    with tf.variable_scope("Transformer", initializer=initializer):
      ...
      encoder_outputs, mu, log_var, latent_sample = self.encode(inputs, src_attention_bias, targets, tgt_attention_bias) # tc modified

      # Generate output sequence if targets is None, or return logits if target
      # sequence is known.
      if targets is None:
        return self.predict(encoder_outputs, src_attention_bias), None, None, None
      else:
        logits = self.decode(targets, encoder_outputs, src_attention_bias)
        return logits, mu, log_var, latent_sample
  ...
  def encode(self, inputs, src_attention_bias, targets=None, tgt_attention_bias=None):
    ...
    with tf.name_scope("encode"):
      ...
      with tf.variable_scope("latent_variable"):
        ...
        latent_sample, mu, log_var = self.latent_variable_layer(src_embedding, tgt_embedding) # get size [batch_size, hidden_size]
      ...
      with tf.variable_scope("contact_latent_varible_to_source"):
        ...
        #print("debug: in encode(), latent_sample_tiled.shape")
        #print(latent_sample_tiled.shape)
        encoder_outputs = tf.concat([src_encoder_outputs, latent_sample_tiled], axis=-1) # get size [batch_size, length, 2*hidden_size]
        #print("debug: in encode(), encoder_outputs.shape 1")
        #print(encoder_outputs.shape)
        encoder_outputs = self.encoder_output_layer(encoder_outputs) # get size [batch_size, length, hidden_size]
        # debug
        #print("debug: in encode(), encoder_outputs.shape 2")
        #print(encoder_outputs.shape)

      # encoder_outputs: size [batch_size, length, hidden_size]
      # latent_sample: size [batch_size, hidden_size]
      # debug
      #assert encoder_outputs.shape[-1].value == self.params["hidden_size"]
      #print('encoder_outputs.shape: ', encoder_outputs.shape)
      #with tf.name_scope("encoder_outputs_shape"):
      #  debug_encoder_outputs = encoder_outputs + tf.zeros_like(encoder_outputs)

      return encoder_outputs, mu, log_var, latent_sample
...
#class LatentVariableLayer(object):
class LatentVariableLayer(tf.layers.Layer):
  ...
  def __init__(self, params, train):
    ...
    if self.train:
      ...
      #self.output_norm_layer_mu = LayerNormalization(params["hidden_size"])
      #self.output_norm_layer_var = LayerNormalization(params["hidden_size"])
      #self.output_norm_layer = LayerNormalization(params["hidden_size"])
  ...
  def __call__(self, src_embedding, tgt_embedding):
    if self.train:
      ...
      mu_var = self.feed_foward_layer(src_tgt_embedding, padding=None) # get size [batch_size, 1, 2 * hidden_size]
      mu_var = tf.squeeze(mu_var, axis = 1) # get size [batch_size, 2 * hidden_size]
      mu, var = tf.split(mu_var, 2, axis = -1) # both with size [batch_size, hidden_size]
      #mu = self.output_norm_layer_mu(mu)
      #var = self.output_norm_layer_var(var)
    else:
      mu = tf.zeros_like(src_embedding)
      var = tf.zeros_like(src_embedding)

    latent_sample = self._sample_gaussian(mu, var) # get size [batch_size, hidden_size]
    #latent_sample = self.output_norm_layer(latent_sample)

    return latent_sample, mu, var

  def _sample_gaussian(self, mu, logvar):
    """
      Args: 
        mu:     size [batch_size, hidden_size]
        logvar: size [batch_size, hidden_size]
      Return:
        z: latent_variable, size [batch_size, hidden_size]
    """
...
class PrePostProcessingWrapper(object):
  ...
  def __init__(self, layer, params, train, input_hidden_size=None, output_hidden_size=None):
    self.layer = layer
  ...
  def __call__(self, x, *args, **kwargs):
    ...
    #print("in PrePostProcessingWrapper.call x.shape")
    #print(x.shape)
    y = self.layer_norm(x)
    ...
    if self.input_hidden_size == self.output_hidden_size:
      return x + y 
    else:
      return y
