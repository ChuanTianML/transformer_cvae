...
class LatentVariableCoreLayer(tf.layers.Layer):
  def __init__(self, params, train, input_size, as_standard_norm=False):
    ...
    for i in range(params["num_latent_layers"]):
      ...
      ffl = ffn_layer.FeedFowardNetwork(
          temp_input_size, params["filter_size"],
          params["relu_dropout"], train, params["allow_ffn_pad"],
          output_size = self.output_size, activation=tf.tanh)
          #output_size = self.output_size, activation=tf.nn.relu)
      ffl = PrePostProcessingWrapper(
          ffl, params, train,
          input_hidden_size = temp_input_size,
          output_hidden_size = self.output_size,
          norm=True, drop=True, residual=(i>0))
    ...
    if self.params["use_std"]:
      self.mu_layer = tf.layers.Dense(
          params["latent_size"], use_bias=False, activation=tf.tanh, name="mu_layer")
      self.std_layer = tf.layers.Dense(
          params["latent_size"], use_bias=False, activation=tf.sigmoid, name="std_layer")

  def __call__(self, inputs):
    """
      Args:
        inputs: with shape [batch_size, hidden_size] 
    """

    # generate standard norm
    if self.as_standard_norm:
      batch_size_value = tf.shape(inputs)[0]
      mu = tf.zeros(shape=[batch_size_value, self.params["latent_size"]])
      if self.params["use_std"]:
        logvar = tf.ones(shape=[batch_size_value, self.params["latent_size"]])
        latent_sample = self._sample_gaussian_v2(mu, logvar)
      else:
        logvar = tf.zeros(shape=[batch_size_value, self.params["latent_size"]])
        latent_sample = self._sample_gaussian(mu, logvar)
      return latent_sample, mu, logvar

    # multi feed forward
    inputs = tf.expand_dims(inputs, axis=1)
    for i, layer in enumerate(self.layers):
      layer_name = "latent_layer_%d" % i
      with tf.variable_scope(layer_name):
        inputs = layer(inputs)
    
    # split
    mu_logvar = tf.squeeze(inputs, axis=1)
    mu, logvar = tf.split(mu_logvar, 2, axis=-1)

    # another feed forward, if use standard deviation
    if self.params["use_std"]:
      with tf.variable_scope("std"):
        logvar = self.std_layer(logvar) * 2.0
...
class LatentVariableLayer(tf.layers.Layer):
  ...
  def __init__(self, params, train):
    ...
    self.prior_latent_layer = LatentVariableCoreLayer(params, train, params["hidden_size"], 
                                                      as_standard_norm=params["use_standard_norm"])
    if self.train:
      self.recog_latent_layer = LatentVariableCoreLayer(params, train, 2*params["hidden_size"])

  def __call__(self, src_embedding, tgt_embedding):

    with tf.variable_scope("prior"):
      latent_sample, prior_mu, prior_logvar = self.prior_latent_layer(src_embedding)
    ...
    with tf.variable_scope("recog"):
      if self.train:
        ...
        latent_sample, recog_mu, recog_logvar = self.recog_latent_layer(src_tgt_embedding)
