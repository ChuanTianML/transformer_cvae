...
from official.transformer.model import transformer4
...
TENSORS_TO_LOG = {
    "ln_rate": "model/get_train_op/learning_rate/learning_rate",
    #"cross_entropy_loss": "model/cross_entropy",
    "tt_loss": "model/final_loss",
    "pr_loss": "model/predict_loss_avg_by_word",
    "kl_loss": "model/kl_loss_avg_by_sent",
    "wtd_kl_loss": "model/weighted_kl_loss_avg_by_sent",
    "bow_loss": "model/bow_loss_avg_by_word",
    }
...
def model_fn(features, labels, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    model = transformer4.Transformer(params, train)
    ...
    words_num = tf.reduce_sum(weights)

    predict_loss = tf.reduce_sum(xentropy)
    predict_loss_avg_by_word = predict_loss / words_num
    tf.identity(predict_loss_avg_by_word, "predict_loss_avg_by_word")
    if train:
      if params["use_std"]:
      ...
      kl_loss_avg_by_sent = kl_loss / real_batch_size
      tf.identity(kl_loss_avg_by_sent, "kl_loss_avg_by_sent")
      # annealing
      ...
      weighted_kl_loss_avg_by_sent = weighted_kl_loss / real_batch_size
      tf.identity(weighted_kl_loss_avg_by_sent, "weighted_kl_loss_avg_by_sent")
      tf.identity(kl_loss_weight, "kl_loss_weight")

      if params["use_bow"]:
        bow_loss = compute_bow_loss(latent_sample, targets, params, train)
        bow_loss_avg_by_word = bow_loss / words_num
        tf.identity(bow_loss_avg_by_word, "bow_loss_avg_by_word") # total loss
        loss = predict_loss + weighted_kl_loss + bow_loss
      else:
        loss = predict_loss + weighted_kl_loss
    else: # eval and infer mode
      loss = predict_loss

    # average loss by sentence or by word
    if 'word' == params["avg_method"]:
      loss = loss / words_num
    elif 'sentence' == params["avg_method"]:
      loss = loss / real_batch_size

    # Save loss as named tensor that will be logged with the logging hook.
    tf.identity(loss, "final_loss") # total loss
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      metric_dict["predict_loss_avg_by_word"] = predict_loss_avg_by_word
      metric_dict["kl_loss_avg_by_sent"] = kl_loss_avg_by_sent
      if params["use_bow"]:
        metric_dict["bow_loss_avg_by_word"] = bow_loss_avg_by_word
      if params["kl_weight"]:
        metric_dict["weighted_kl_loss_avg_by_sent"] = weighted_kl_loss_avg_by_sent
...
def compute_kl_loss(mu, log_var):
  ...
  """
  standard_mu = tf.zeros_like(mu)
  standard_log_var = tf.zeros_like(log_var)
  kl_loss = gaussian_kld(mu, log_var, standard_mu, standard_log_var) # get size [batch_size, hidden_size]
  return kl_loss
...
def compute_bow_loss(latent_sample, targets, params, train):
  ...
  with tf.variable_scope("bow_decoder"):
    ...
    bow_ffn_layer = ffn_layer.FeedFowardNetwork(
        params["latent_size"], params["filter_size"],
        params["relu_dropout"], train, params["allow_ffn_pad"],
        output_size = params["vocab_size"], activation=tf.nn.relu 
    )
    expd_lv = tf.expand_dims(latent_sample, axis=1) # get [batch_size, 1, hidden_size]
    bow_logits = bow_ffn_layer(expd_lv, padding=None) # get [batch_size, 1, vocab_size]
    length = tf.shape(targets)[1]
    tile_bow_logits = tf.tile(bow_logits, [1, length, 1]) # get [batch_size, length, vocab_size]

    # compute loss
    xentropy, weights = metrics.padded_cross_entropy_loss(
        tile_bow_logits, targets, params["label_smoothing"], params["vocab_size"])
